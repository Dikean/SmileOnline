...
<div class="card shadow">
            ...
            <div class="card-body">
               ...
               {{session('notification')}}
                ...
                @if($role == 'paciente')
                <p>Se cancelará tu cita reservada con el medico <b>{{ $appointment->doctor->name}}</b>
                    (especialidad <b>{{ $appointment->specialty->name}}</b>) 
                    para el día <b>{{ $appointment->sheduled_date}}</b></p>

                @elseif($role == 'doctor')

                <p>Se cancelará la cita medica del paciente <b>{{ $appointment->patient->name}}</b>
                    (especialidad <b>{{ $appointment->specialty->name}}</b>) 
                    para el día <b>{{ $appointment->sheduled_date}}</b>
                    , la hora <b> {{ $appointment->sheduled_time_12}} </b>.</p>
                @else
                <p>Se cancelará la cita medica del paciente <b>{{ $appointment->patient->name}}</b>
